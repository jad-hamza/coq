...
let autorewrite ?(conds=Naive) fast tac_main lbas =
  Tacticals.New.tclREPEAT_MAIN (Proofview.tclPROGRESS
    (List.fold_left (fun tac bas ->
       Tacticals.New.tclTHEN tac
        (one_base (fun dir c tac ->
	  let tac = (tac, conds) in
	    general_rewrite fast dir AllOccurrences true false ~tac (EConstr.of_constr c))
	  tac_main bas))
      (Proofview.tclUNIT()) lbas))

let autorewrite_multi_in ?(conds=Naive) fast idl tac_main lbas =
  Proofview.Goal.enter begin fun gl ->
 (* let's check at once if id exists (to raise the appropriate error) *)
  ...
  let general_rewrite_in id dir cstr tac =
    ...
    general_rewrite_in fast dir AllOccurrences true ~tac:(tac, conds) false id cstr false
  in
...
let autorewrite_in ?(conds=Naive) fast id = autorewrite_multi_in ~conds fast [id]

let gen_auto_multi_rewrite conds fast tac_main lbas cl =
  let try_do_hyps treat_id l =
    autorewrite_multi_in ~conds fast (List.map treat_id l) tac_main lbas
  in
  ...
  then
    ...
    in
    compose_tac
	(if cl.concl_occs != NoOccurrences then autorewrite ~conds fast tac_main lbas else Proofview.tclUNIT ())
	(match cl.onhyps with
...
let auto_multi_rewrite ?(conds=Naive) fast lems cl =
  Proofview.V82.wrap_exceptions (fun () -> gen_auto_multi_rewrite conds fast (Proofview.tclUNIT()) lems cl)

let auto_multi_rewrite_with ?(conds=Naive) fast tac_main lbas cl =
  ...
  match onconcl,cl.Locus.onhyps with
	...
	*)
	Proofview.V82.wrap_exceptions (fun () -> gen_auto_multi_rewrite conds fast tac_main lbas cl)
    | _ ->
