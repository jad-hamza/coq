	...
	tclTHENLIST
	  [
	    ...
	    (* replacing [res] with its value *)
            observe_tac "rewriting res value" (Proofview.V82.of_tactic (Equality.rewriteLR false (mkVar hres)));
	    (* Conclusion *)
		  ...
		  | App(eq,args) when (EConstr.eq_constr sigma eq eq_ind)  ->
			...
			tclTHENLIST [ Proofview.V82.of_tactic (Simple.intro id);
				     generalize_dependent_of (destVar sigma args.(1)) id;
                                     tclTRY (Proofview.V82.of_tactic (Equality.rewriteLR false (mkVar id)));
				     intros_with_rewrite
				   ]
			  g
			...
			tclTHENLIST [ Proofview.V82.of_tactic (Simple.intro id);
				     generalize_dependent_of (destVar sigma args.(2)) id;
                                     tclTRY (Proofview.V82.of_tactic (Equality.rewriteRL false (mkVar id)));
				     intros_with_rewrite
				   ]
			  g
		      else
    ...
    *)
	...
	tclTHENLIST[
	  tclMAP (fun id -> Proofview.V82.of_tactic (Simple.intro id)) ids;
          Proofview.V82.of_tactic (Equality.rewriteLR false (mkConst eq_lemma));
